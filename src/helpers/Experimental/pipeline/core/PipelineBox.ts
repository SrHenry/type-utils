...
export class PipelineBox<T> {
    ...
    // biome-ignore lint/suspicious/noShadow: type parameter T intentionally shadows class generic
    ...
    pipe<U>(fn: (value: T) => U | Promise<U>): PipelineBox<U> | AsyncPipelineBox<U> {
        if (isCallWithTransform(fn)) {
            ...
            if (result instanceof PipelineBox || result instanceof AsyncPipelineBox)
                return result.depipe() as PipelineBox<U> | AsyncPipelineBox<U>
            if (result instanceof Promise)
                return new AsyncPipelineBox(result as Promise<U>) as AsyncPipelineBox<U>
            return new PipelineBox(result as U) as PipelineBox<U>
        }
        const result = fn(this._value)
        if (result instanceof PipelineBox || result instanceof AsyncPipelineBox)
            return result as PipelineBox<U> | AsyncPipelineBox<U>
        if (checkPipeTransform(result)) {
            const inner = (result as { depipe(): unknown }).depipe()
            if (inner instanceof PipelineBox || inner instanceof AsyncPipelineBox)
                return inner as PipelineBox<U> | AsyncPipelineBox<U>
...
export class AsyncPipelineBox<T> {
    ...
    tapAsync(
        ...
        return new AsyncPipelineBox(
            this._promise.then(async value => {
                try {
                    await fn(value)
                } catch (error) {
                    handleTapError(error, options)
