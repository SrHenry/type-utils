...
describe('enpipe', () => {
    ...
    it('should enpipe a value as bypass for defined pipeline', async () => {
        ...
        const addPostCurried = (post: Record<string, any>) => (id: string) =>
            pipe(addPostFactory).pipe(callWith(db)).pipe(callWith(id, post)).depipe()
    ...
    it('should enpipe multiple values in pipeline callback factory function', async () => {
        ...
        const addPostCurried = (post: Record<string, any>) => (id: string) =>
            createPipeline(addPostFactory)
                ...
                .pipeAsync(
                    (r: unknown) =>
                        new Promise<boolean>(resolve =>
                            setTimeout(() => resolve(r as boolean), Math.random() * 1000)
                        )
                )
                .depipe()
        ...
        const underparameterizedPipelineCallback__2 = enpipe(
            (a: number, b: number, c: number) => a + b + c,
            1
        ).pipe((fn: (b: number) => (c: number) => number) =>
            enpipe(fn, 2)
        ) as unknown as PipelineBox<(c: number) => number>
...
describe('createPipeline', () => {
    it('should create a pipeline', () => {
        const atob = (s: string) =>
            Buffer.from(s, 'base64').toString() as string & {
                __tag: 'atob'
            }
            ...
            pipe('hello')
                .pipe(
                    tap(
                        ...
                        { swallow: false, catch: e => errors.push(e as Error) }
                    )
                )
                .depipe()
        ).toThrow('tap-fail')
...
describe('tapAsync factory', () => {
    ...
    it('should return a valid async pipeline function from tapAsync(fn).catch(handler)', async () => {
        const errors: Error[] = []
        const tapAsyncFn = tapAsync(async (_s: string) => {
            throw new Error('fail')
        }).catch(e => errors.push(e as Error))
            ...
            .pipeAsync(
                tapAsync(
                    ...
                    },
                    {
                            ...
                            db.audit.push({ event: 'audit_error', error: (e as Error).message }),
                    }
                )
            )
            .pipeAsync(id => insertPost(id, newPost))
...
describe('.tap()', () => {
    it('should be equivalent to .pipe(tap(fn))', () => {
        ...
        const fluent = pipe('hello')
            .tap(v => logFluent.push(v))
            .depipe()
        const factory = pipe('hello')
            .pipe(tap(v => logFactory.push(v)))
            .depipe()
    ...
    it('should route tapAsync errors to the catch handler', async () => {
        ...
        const result = await pipe(Promise.resolve('hello'))
            .tapAsync(
                ...
                { catch: (e: unknown) => errors.push(e as Error) }
            )
            .pipeAsync(s => s.toUpperCase())
            .depipe()
    ...
    it('should call the catch handler before rejecting when both catch and swallow:false are set on tapAsync', async () => {
        ...
        const result = await pipe(Promise.resolve('hello'))
            .tapAsync(
                ...
                { swallow: false, catch: (e: unknown) => errors.push(e as Error) }
            )
            .pipeAsync(s => s.toUpperCase())
            .depipe()
            .catch((e: Error) => e.message)
