...
- [ ] Restore `yarn docs` (typedoc) under TypeScript 7
    - **ID**: typedoc-ts7-gap
    - **Workaround**: typedoc Pages workflow `.github/workflows/typedoc-github-pages.yml` renamed to `.disabled` (this commit) to stop the failing-run churn (6 consecutive failures between 2026-07-09 and 2026-07-10 since TS7 landed on master via PR #72). Re-enabling is `git mv` back to `.yml` after this task is closed.
    ...
    - **Files**: `package.json`, `yarn.lock`, (optionally) `typedoc.json`, `.github/workflows/typedoc-github-pages.yml.disabled`
    - **Plan**:
        ...
        3. If only dev-pre-releases are available: stay on TS 6 for typedoc via `@typescript/typescript6` (alias `typescript` to `npm:@typescript/typescript6` in a `typedoc-only` lockfile subtree using Yarn 1's `resolutions` `npm:` aliasing, or invoke typedoc with `node --require` shim that monkey-patches `require('typescript')` to return TS 6 inside the typedoc subprocess). Also requires a typedoc-specific tsconfig override that strips `stableTypeOrdering` (TS 7.0-only) since TS 6 doesn't recognize it.
        4. Re-add `yarn docs` to `ci.yml` (`yarn docs` is currently NOT in `.github/workflows/ci.yml` — only `tsc`, `check`, `test`, `circular-dependencies`, `build:clean` are), and rename `typedoc-github-pages.yml.disabled` back to `.yml` to re-enable the Pages deployment.
    - **Acceptance**: `yarn docs` runs to completion under TS 7.0.2 (exit 0, HTML generated in `./docs`); the CI `.github/workflows/ci.yml` optionally re-runs `yarn docs` (or skip if API-doc regeneration is a release-only concern); the typedoc-github-pages workflow is re-enabled (`git mv` back to `.yml`) and runs to completion on a push to master; the worktree does not regress when TS 7.1 launches with a stable programmatic API.
